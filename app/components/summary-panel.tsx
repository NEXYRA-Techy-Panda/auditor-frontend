...
export default function SummaryPanel({
  ...
  dataset,
  onSummaryChange,
  onTariffMutationStart,
  onTariffCommitted,
  refreshToken = 0,
  suppressPrintSummary = false,
}: {
  backendUrl: string;
  datasetId: string | null;
  dataset: DatasetItem | null;
  onSummaryChange?: (summary: DatasetSummary | null) => void;
  onTariffMutationStart?: () => void;
  onTariffCommitted?: (datasetId: string) => void;
  refreshToken?: number;
  ...
  useEffect(() => {
    if (refreshToken <= 0 || !datasetId) return;
    inFlight.current?.abort();
    inFlight.current = null;
    const timer = setTimeout(() => void load(datasetId), 0);
    return () => clearTimeout(timer);
  }, [datasetId, load, refreshToken]);
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      // The parent refresh token reloads the selected summary and dependent
      // server-derived costs after the shared tariff mutation commits.
    } catch (err) {
      if (!mounted.current) return;
  ...
  }, [backendUrl, datasetId, onSummaryChange, onTariffCommitted, onTariffMutationStart, rateInput, saving]);

  useEffect(() => {
    if (!snapshot || suppressPrintSummary) return;
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 border-t border-zinc-200 pt-4 dark:border-zinc-800">
        <button
          type="button"
          disabled={!eligibility.eligible || suppressPrintSummary}
          onClick={openPrint}
          className="rounded-full bg-zinc-900 px-5 py-2 text-sm font-medium text-white transition-colors hover:bg-zinc-700 disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600 dark:bg-zinc-50 dark:text-zinc-900 dark:hover:bg-zinc-300"
        >
          Print summary
      ...
      {!suppressPrintSummary && <ReportView snapshot={snapshot} />}
    </section>
  );
}
