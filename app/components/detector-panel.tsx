...
export default function DetectorPanel({ backendUrl, datasetId, dataset, summary }: { backendUrl: string; datasetId: string | null; dataset: DatasetItem | null; summary: DatasetSummary | null }) {
  ...
  const resetJob = useCallback(() => { stopPolling(); inFlight.current?.abort(); inFlight.current = null; tracker.current.issue(); setJobId(null); setJob(null); setPage(1); setJobError(null); }, [stopPolling]);
  ...
  }, [backendUrl, datasetId, detector, stopPolling]);
  ...
  const create = useCallback(async () => {
    ...
    try {
      ...
      setJobId(ack.job_id); setJob(null);
  ...
  }, [availableDetectors.length, backendUrl, creating, datasetId, detector, evaluationEnd, evaluationStart, referenceEnd, referenceStart]);
