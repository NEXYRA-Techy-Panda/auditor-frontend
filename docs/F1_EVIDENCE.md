# F1_EVIDENCE — auditor-frontend (contract mirror)
...
- `Python 3.13.15` (64-bit) at
  ...
  pip `26.2.1`, SQLite `3.50.4`, venv OK. PATH shims stale; PATH untouched.
- User-verified + agent-verified; dependency compatibility is F2. This repo
  needs no Python; F1 needs no Python.
...
scripts/verify-contract.mjs, .gitignore. Short hashes: CONTRACT `a10dc136…`,
schema `dd4feedd…`, CSV `4e217d7b…`, API `49e54a7d…`, ref.json `14dec040…`,
ref.csv `f7f6883a…`, expected `522fb8dc…`, verify `c6f10494…`.
...
interval, `meta_run`/`meta_policy` the only JSON columns, aligned-grid
coverage, no total rows. Half-open UTC intervals; missing ≠ zero;
policy versions immutable; tolerance 1e-6 kWh; tariff never alters identity.
